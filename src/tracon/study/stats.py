"""Small, dependency-free statistics used by every analysis.

Every rate reported in the study carries a Wilson 95% interval, and every
comparison between two rates carries a two-proportion test. Nothing here is
clever; it is here so that no number in the writeup is a bare point estimate.
"""
...
def cochran_armitage_trend(rows: list[tuple[float, int, int]]) -> dict:
    """Trend test for a proportion across ordered bins.

    ``rows`` is [(score, successes, total), ...] with ``score`` the bin's
    ordinal position (here: context-window size). This is the test behind the
    study's negative result on context pressure — a flat-looking curve is only
    worth reporting as "flat" if the trend statistic agrees.
    """
    rows = [r for r in rows if r[2] > 0]
    if len(rows) < 3:
        return {"z": 0.0, "p": 1.0, "note": "too few bins"}
    n_total = sum(t for _, _, t in rows)
    s_total = sum(s for _, s, _ in rows)
    p_bar = s_total / n_total
    if p_bar in (0.0, 1.0):
        return {"z": 0.0, "p": 1.0, "note": "degenerate"}
    t_mean = sum(t * score for score, _, t in rows) / n_total
    numerator = sum(s * (score - t_mean) for score, s, _ in rows)
    variance = p_bar * (1 - p_bar) * sum(t * (score - t_mean) ** 2 for score, _, t in rows)
    if variance <= 0:
        return {"z": 0.0, "p": 1.0, "note": "degenerate"}
    z = numerator / math.sqrt(variance)
    return {"z": z, "p": 2 * normal_sf(abs(z))}


def quantiles(values: list[float], points: tuple[float, ...] = (0.5, 0.9, 0.95, 0.99)) -> dict:
    """Nearest-rank quantiles. Empty input yields an empty dict, never a crash."""
    if not values:
        return {}
